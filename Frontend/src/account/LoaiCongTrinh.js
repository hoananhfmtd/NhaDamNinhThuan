...
const List = () => {
    ...
    const getList = React.useCallback(async ({ datas, limit, page, signal }) => {
        ...
        const response = await new Api().getAllLoaiCongTrinh({
    ...
    const handleDelete = React.useCallback(
        async (row) => {
            ...
            try {
                const response = await new Api().deleteLoaiCongTrinh(row.data.maMuc);
    ...
    const handleUpsert = async (data) => {
        try {
            const result = await new Api().upsertLoaiCongTrinh({
                ...data,
            });
            if (result?.code === 200) {
                setPage(0);
                ...
                ThongBao({
                    status: 'success',
                    message: `${status === 'new' ? 'Thêm mới' : 'Cập nhật'
                        } loại công trình thành công!`,
                });
            }
        } catch (error) {
            console.log(error);
    ...
    const handleGetDetail = async (data, isEdit) => {
        try {
            const result = await new Api().getLoaiCongTrinh(data);
            if (result?.code === 200) {
