...
export default function BieuMauCountByNguoiTao() {
    const [reports, setReports] = React.useState([{ nguoiTao: "", soLuong: 0 }]);
    ...
    const handleThongKe = async () => {
        ...
        //nếu coQuanThucHienIds ===[] thì gán coQuanThucHienIds = null
        _filtered.loaiThongKe = 1;
        if (!filtered.bieuMau && scope !== 'tinh') {
            _filtered.bieuMau = "BieuMauSo1";
        }
        if (tinhThanhId && scope === 'tinh') {
            _filtered.tinhThanhId = tinhThanhId;
            if (!_filtered.bieuMau) {
                _filtered.bieuMau = "BieuMauSo2";
            }
        }
        ...
        try {
            ...
            if (response?.code === 200) {
                ...
                if (response?.data.length === 0) {
                    data = [{ nguoiTao: "Không có dữ liệu", soLuong: 0 }];
                }
                if (scope === 'tinh') {
                    setReports(data);
                }
                // nếu data rỗng setReports = 0
                else {
                    setReports(data);
    ...
    const CustomTooltipNguoiTao = ({ active, payload, label }) => {
        if (active && payload && payload.length) {
            return (
                <div style={{ background: '#000000ba', borderRadius: 5, padding: 10, color: '#fff' }}>
                    {/* <div>{`Tên ${payload[0].payload.name}`}</div> */}
        ...
        <div style={{ border: '1px solid black !important' }} >
            ...
            < Box mt={5} style={{ width: '97.5%', marginLeft: '23px' }}>
                <div style={{ borderRadius: '8px' }}>
                    <Box >
                        ...
                        <Box display="flex" gap={3} justifyContent="right" pt={5} style={{ backgroundColor: '#fff ', borderRadius: ' 10px 10px 0 0', position: 'relative' }}>
                            <LocalizationProvider dateAdapter={AdapterDayjs} adapterLocale={vi} localeText={viVN.components.MuiLocalizationProvider.defaultProps.localeText}>
                                <Typography sx={{ fontFamily: 'Inter', fontWeight: 'bold', fontSize: '16px', position: 'absolute', left: '40px', top: "40px" }}>THỐNG KÊ BIỂU MẪU THEO NGƯỜI TẠO</Typography>
                                ...
                                <>
                                    ...
                                    <MySelect
                                        ...
                                        width="200px"
                                        name="tenBieuMau"
                                        value={filtered?.tenBieuMau || filteredListBM[0]}
                                        onChange={(e, value) => {
                                            setFiltered((prev) => ({
                                                ...prev,
                    ...
                    <div style={{ height: 450, backgroundColor: '#fff', borderRadius: ' 0 0  10px 10px', padding: '30px' }}>
                        <>
                            {/* //Biểu đồ chấm */}
                            {/* <ResponsiveContainer>
                                <ScatterChart
                                    data={reports
                                        .filter(report => report.nguoiTao && report.soLuong)
                                        .sort((a, b) => a.index - b.index)
                                        .map(({ nguoiTao, soLuong }) => ({
                                            name: nguoiTao,
                                            uv: soLuong,
                                        }))
                                    }
                                ...
                                >
                                    ...
                                    <XAxis
                                        ...
                                        tickLine
                                        tick={{ fontSize: 10 }}
                                        label={{ value: 'Cán bộ điều tra', position: 'bottom', offset: 0, fill: '#000' }}
                                    />
                                    <YAxis dataKey="uv" name="Số lượng" label={{ value: 'Số lượng', angle: -90, position: 'insideLeft', fill: '#000' }} />
                                    <Tooltip content={<CustomTooltipNguoiTao />} />
                                    <Legend verticalAlign="bottom" align="right" content={<RenderLegend />} />
                                    <Scatter fill="#b0d8ee">
                                        <LabelList dataKey="uv" position="top" />
                                    </Scatter>
                                </ScatterChart>
                                {reports.some(item => !item.nguoiTao && item.soLuong === 0) && (
                                    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                                        <CircularProgress size={24} style={{ marginRight: '10px' }} />
                                        <span style={{ color: '#1976d2', fontSize: '16px', margin: '5px' }}>
                                            Đang tải dữ liệu...
                                        </span>
                                    </div>
                                )}
                            ...
                            <ResponsiveContainer >
                                ...
                                >
                                    ...
                                    <XAxis dataKey="name" axisLine={true} tickLine={true} label={{ value: '', position: 'bottom', offset: 0, fill: '#000' }} />
