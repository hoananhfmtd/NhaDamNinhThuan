...
export default function BieuMauCountByDate() {
    const [reports, setReports] = React.useState([{ ngayThang: 1722790800, soLuong: 0 }]);
        ...
        try {
            ...
            if (response?.code === 200) {
                ...
                if (response?.data.length === 0) {
                    data = [{ ngayThang: 1722790800, soLuong: 0 }];
                }
                if (scope === 'tinh') {
                    setReports(data);
                }
                    ...
                    <Box >
                        ...
                        <Box display="flex" gap={3} justifyContent="right" pt={5} style={{ backgroundColor: '#fff ', borderRadius: ' 10px 10px 0 0', position: 'relative' }}>
                            <LocalizationProvider dateAdapter={AdapterDayjs} adapterLocale={vi} localeText={viVN.components.MuiLocalizationProvider.defaultProps.localeText}>
                                <Typography sx={{ fontFamily: 'Inter', fontWeight: 'bold', fontSize: '16px', position: 'absolute', left: '40px', top: "40px" }}>THỐNG KÊ BIỂU MẪU THEO THỜI GIAN</Typography>
                            ...
                            <ResponsiveContainer>
                                ...
                                >
                                    ...
                                    <XAxis
                                        ...
                                        label={{ value: 'Ngày tháng', position: 'bottom', offset: 0, fill: '#000' }}
                                    />
                                    <YAxis dataKey="uv" name="Số lượng" label={{ value: 'Số lượng', angle: -90, position: 'insideLeft', fill: '#000' }} />
                                    <Tooltip content={<CustomTooltipNgayTao />} />
                                    <Legend verticalAlign="bottom" align="right" content={<RenderLegend />} />
                                    <Line type="monotone" dataKey="uv" stroke="#b0d8ee" dot={true} />
                                </LineChart>
                                {reports.some(item => item.ngayThang === 1722790800 && item.soLuong === 0) && (
                                    ...
                                    <BarChart
                                        data={
                                            filtered.day === 7 ? day7 : day30
                                                .map((dayItem) => ({
                                                    name: dayItem.day,
                                                    uv: reports.find((i) => dayjs(i?.ngayTao).format('DD/MM/YYYY') === dayItem.day)?.soLuong || 0,
                                                }))}
                                        margin={{ top: 20, right: 30, left: 20, bottom: 10 }}
                                        title="Biểu đồ số lượng biểu mẫu được tạo theo thời gian"
                                    >
